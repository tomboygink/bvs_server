...
export class Devs_groupsTable {
	...
	async selectDevsGroups() {
		...
		var dev: any = {
			id: 0, 				//идентификатор
			group_dev_id: '',   //идентификатор группы 
		...
		for (var i in roots_gr.rows) {
			//Запрос на получение устройств по группам 
	...
	async updateDevsGroups() {
		...
		for (var i = 0; i < data.rows.length; i++) {
			...
			// //Получение скважин
			var data_well = await this.db.query("SELECT * FROM skvazhiny WHERE group_id = " + data.rows[i].id);
			// //Редактирование скважин при изменении организации 
			for (var j = 0; j < data_well.rows.length; j++) {
				await this.db.query("UPDATE skvazhiny SET org_id =" + this.args.org_id + " WHERE id = " + data_well.rows[j].id);
			}
		}
	}
}
